#!/usr/bin/env perl
# (C) 2014, Trevor Woerner <trevor.woerner@linaro.org>
#           edited to work for OpenEmbedded
...
my @oe_chief = ();
push(@oe_chief, "Richard Purdie:richard.purdie\@linux-foundation.org");

my @oe_chief_names = ();
foreach my $chief (@oe_chief) {
    if ($chief =~ m/^(.*):(.*)/) {
	my $chief_name = $1;
	my $chief_addr = $2;
	push(@oe_chief_names, $chief_name);
    }
}
my $oe_chiefs = "\(" . join("|", @oe_chief_names) . "\)";
...
push(@signature_tags, "Reported-by:");
push(@signature_tags, "Cc:");
...
if (!GetOptions(
		...
		'git-oe-chiefs!' => \$email_git_oe_chiefs,
...
if ($email &&
    ($email_maintainer + $email_list + $email_subscriber_list +
     $email_git + $email_git_oe_chiefs + $email_git_blame) == 0) {
...
if (open (my $maint, '<', "${lk_path}MAINTAINERS")) {
        while (<$maint>) {
                my $line = $_;
                ...
                if ($line =~ m/^(\C):\s*(.*)/) {
                        ...
                        if ($type eq "F" || $type eq "X") {
                                ...
                                $value =~ s/\*/\.\*/g;       ##Convert * to .*
                                $value =~ s/\?/\./g;         ##Convert ? to .
                                ##if pattern is a directory and it lacks a
...
MAINTAINER field selection options:
  --email => print email address(es) if any
    ...
    --git-oe-chiefs => include ${oe_chiefs}
...
Notes:
  ...
  File ".get_maintainer.conf", if it exists in the source
...
sub vcs_find_signers {
    ...
    if (!$email_git_oe_chiefs) {
	@signatures = grep(!/${oe_chiefs}/i, @signatures);
    }
